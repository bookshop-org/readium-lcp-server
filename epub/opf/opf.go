...
var unsupportedXMLDeclaration = regexp.MustCompile(`^[\p{Cf}\p{Zs}\s]*<\?[\p{Cf}\p{Zs}\s]*xml[\p{Cf}\p{Zs}\s]+version[\p{Cf}\p{Zs}\s]*=[\p{Cf}\p{Zs}\s]*"[\p{Cf}\p{Zs}\s]*1.[1-9][\p{Cf}\p{Zs}\s]*"`)
